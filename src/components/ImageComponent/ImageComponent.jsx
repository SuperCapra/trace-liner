...
import Modal from '../ModalExport/ModalExport.jsx'
...
import DesignBySVG from '../../assets/images/logoImage.svg?react'
import LogoInternalSVG from '../../assets/images/logoInternal.svg?react'
...
function ImageComponent(props) {
  ...
  const pregenerateImagePng = useCallback(async function generate(bj, anchor, scale = 10, attempt = 0) {
    const MAX_RETRIES = 2
    addOpacity()
    ...
    //         return generate(bj, anchor, 2, attempt + 1);
    //       }
    //     } else {
    ...
    try {
      ...
      canvas.toBlob(async function(blob) {
        if (!blob) {
          insertLogsModal({body: apiUtils.getErrorLogsBody(visitId,'Exception: blob null from canvas.toBlob for png',JSON.stringify(infoLog),'Imagecomponent','pregenerateImagePng','exception')})
          if (attempt < MAX_RETRIES) {
            await new Promise(res => setTimeout(res, 300))
            return generate(bj, anchor, 2, attempt + 1)
          }
  ...
  const pregenerateImageJpeg = useCallback(async function (scale) {
  ...
  const drawLinePatternTripleLineDotted = useCallback((coordinates, lengthCoordinates, resolutionUsing, ctx, color, width, height, mapCenter, minY, maxY, mode5Enabled, zoomFactor, dimentionCircleFinish) => {
    ...
    ctx.stroke()
  },[drawLoopNoCircle, drawOverlapCircles])
  ...
  const drawLine = useCallback(async (color, canvasWidth, canvasHeight, resolutionChanging, mode4Enabled, mode5Enabled, r1, r2, r3) => {
    ...
    drawLinePatternBrokenTripleLine
  ])
  ...
  const handleClickDispatcher = (data) => {
    ...
    if(data.type === 'downloadshare') {
      ...
      let ratioText = '9:16'
      console.info('ratioText:', ratioText)
