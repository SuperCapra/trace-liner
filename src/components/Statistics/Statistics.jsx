...
import MultiDropdown from '../MultiDropdown/MultiDropdown.jsx';
import Table from '../Table/Table.jsx';
import TextCheckbox from '../TextCheckbox/TextCheckbox.jsx'
import Refresh from '../../assets/images/refresh.svg?react'
import CopyExcel from '../../assets/images/copyExcel.svg?react'
import brandingPalette from '../../config/brandingPalette.js';
import queryUtils from '../../utils/queryUtils.js';
import statisticsUtils from '../../utils/statisticsUtils.js';
...
function Statistics(props) {
    ...
    const retrieveTables = () => {
        dbInteractions.processQuery(queries.tables, import.meta.env.VITE_JWT_TOKEN).then(res => {
            let tableArray = []
    ...
    const retrieveNumbers = () => {
        dbInteractions.processQuery(queries.getQueryCount('visits'), import.meta.env.VITE_JWT_TOKEN).then(res => {
            console.log('res:',res)
            if(res.records) {
        ...
        dbInteractions.processQuery(queries.getQueryCount('users'), import.meta.env.VITE_JWT_TOKEN).then(res => {
            console.log('res:',res)
            if(res.records) {
        ...
        dbInteractions.processQuery(queries.getQueryCountFilter('users', 'HAS_STRAVA = true'), import.meta.env.VITE_JWT_TOKEN).then(res => {
            console.log('res:',res)
            if(res.records) {
                setNumberStravaUsers(Number(res.records[0].count))
            }
        }).catch(e => {
            console.error('error querying number of visits:', e)
        })
        dbInteractions.processQuery(queries.getQueryCountFilter('users', 'has_strava = true AND lastmodified_at >= now() - INTERVAL\'30 days\''), import.meta.env.VITE_JWT_TOKEN).then(res => {
            console.log('res:',res)
            if(res.records) {
        ...
        dbInteractions.processQuery(queries.getQueryCountFilter('users', 'has_strava = true AND lastmodified_at >= now() - INTERVAL\'60 days\' AND (lastmodified_at < now() - INTERVAL\'30 days\' OR created_at < now() - INTERVAL\'30 days\')'), import.meta.env.VITE_JWT_TOKEN).then(res => {
            console.log('res:',res)
            if(res.records) {
        ...
        dbInteractions.processQuery(queries.getQueryCountFilter('users', 'has_strava = true AND created_at >= now() - INTERVAL\'30 days\''), import.meta.env.VITE_JWT_TOKEN).then(res => {
            console.log('res:',res)
            if(res.records) {
        ...
        dbInteractions.processQuery(queries.getQueryCountFilter('users', 'has_strava = true AND created_at < now() - INTERVAL\'30 days\' AND created_at >= now() - INTERVAL\'60 days\''), import.meta.env.VITE_JWT_TOKEN).then(res => {
            console.log('res:',res)
            if(res.records) {
        ...
        dbInteractions.processQuery(queries.getQueryCountFilter('exports', 'timestamp >= now() - INTERVAL\'30 days\''), import.meta.env.VITE_JWT_TOKEN).then(res => {
            console.log('res:',res)
            if(res.records) {
        ...
        dbInteractions.processQuery(queries.getQueryCountFilter('exports', 'timestamp < now() - INTERVAL\'30 days\' AND timestamp >= now() - INTERVAL\'60 days\''), import.meta.env.VITE_JWT_TOKEN).then(res => {
            console.log('res:',res)
            if(res.records) {
        ...
        dbInteractions.processQuery(queries.getQueryCount('activities'), import.meta.env.VITE_JWT_TOKEN).then(res => {
            console.log('res:',res)
            if(res.records) {
        ...
        dbInteractions.processQuery(queries.getQueryCount('exports'), import.meta.env.VITE_JWT_TOKEN).then(res => {
            console.log('res:',res)
            if(res.records) {
    ...
    const retrieveColumns = (nameTable) => {
        ...
        dbInteractions.processQuery(queries.getQueryColumns(nameTable), import.meta.env.VITE_JWT_TOKEN).then(res => {
            let columnArray = []
            let columnsData = {}
            if(res.records.length) {
                res.records.forEach(e => {
    ...
    const launchRefresh = () => {
        ...
        dbInteractions.processQuery(query, import.meta.env.VITE_JWT_TOKEN).then(res => {
            console.log('res:', res)
    ...
    useEffect(() => {
        const handleResize = () => {resizeInputConstrain();};
