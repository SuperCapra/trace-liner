...
import LoaderLogo from './components/LoaderLogo/LoaderLogo.jsx'
...
import DesignedSVG from './assets/images/designed.svg?react'
import InstagramYellowSVG from './assets/images/buttonInstagramYellow.svg?react'
import './App.css'

const ImageComponent = lazy(() => import('./components/ImageComponent/ImageComponent.jsx'));
const Statistics = lazy(() => import('./components/Statistics/Statistics.jsx'));

let stravaAuthorizeUrl = import.meta.env.VITE_STRAVA_HOST + import.meta.env.VITE_STRAVA_AUTORIZE_DIRECTORY + 
...
let accessToken
let isLoading = false
// let isAutoScrolling = false //TODO decide how to use the auto scrolling
...
class Homepage extends React.Component{
    ...
    try {
      if(event && event.target && event.target.files && event.target.files.length) {
        ...
        reader.onload = (e) => {
          ...
          let elevationGain = utils.computeElevationGain(elevationStream)
          let coordinatesStream = trkpt && trkpt.length ? [...trkpt.map(point => ([Number(point.lon), Number(point.lat)]))] : undefined
          let distance = coordinatesStream ? utils.getTotalDistance(coordinatesStream) : undefined
          let durationData =  utils.computeDuration(trkpt)
          logUtils.loggerText('json: ', json)
          logUtils.loggerText('gpx:', gpx)
          let activityPreparing = {
            average: undefined,
            pace: undefined,
            altitudeStream: elevationStream,
            metric: {
              beautyAverage: undefined,
              beautyPace: undefined,
              beautyElevation: elevationGain ? elevationGain + 'm' : undefined,
              beautyDistance: distance ? (distance / 1000).toFixed(0) + 'km' : undefined,
              distance: distance ? Number((distance / 1000).toFixed(0)) : undefined,
              subtitle: undefined
            },
            imperial: {
              beautyAverage: undefined,
              beautyPace: undefined,
              beautyElevation: elevationGain ? (elevationGain * 3.28084).toFixed(0) + 'ft' : undefined,
              beautyDistance: distance ? ((distance / 1000) * 0.621371).toFixed(0) + 'mi' : undefined,
              distance: distance ? Number(((distance / 1000) * 0.621371).toFixed(0)) : undefined,
              subtitle: undefined
            },
            beautyCoordinates: undefined,
            beautyEndCoordinates: undefined,
            beautyDuration: durationData.elapsedTime ? utils.getBeautyDuration(durationData.elapsedTime) : undefined,
            beautyName: trk.name ? he.decode(trk.name) : undefined,
            beautyNameNoEmoji: trk.name ? utils.removeEmoji(he.decode(trk.name)) : undefined,
            beautyPower: undefined,
            beautyDatetimeLanguages: durationData.startLocalDateTime ? utils.getBeautyDatetime(durationData.startLocalDateTime) : undefined,
            coordinates: coordinatesStream,
            durationMoving: durationData.movingTime ? durationData.movingTime : undefined,
            durationElapsed: durationData.elapsedTime ? durationData.elapsedTime : undefined,
            endLatitude: trkpt && trkpt.length && trkpt[trkpt.length - 1].lat ? trkpt[trkpt.length - 1].lat : undefined,
            endLongitude: trkpt && trkpt.length && trkpt[trkpt.length - 1].lon ? trkpt[trkpt.length - 1].lon : undefined,
            distance: distance,
            distanceStream: [],
            elevation: elevationGain ? elevationGain : undefined,
            locationCountry: undefined,
            movingTime: durationData.movingTime,
            timingStreamSeconds: [],
            name: trk.name ? he.decode(trk.name) : undefined,
            photoUrl: undefined,
            sportType: trk.type ? trk.type : undefined,
            startDate: durationData.startDate ? durationData.startDate : undefined,
            startDateLocal: durationData.startLocalDate ? durationData.startLocalDate : undefined,
            startLatitude: trkpt && trkpt.length && trkpt[0].lat ? trkpt[0].lat : undefined,
            startLongitude: trkpt && trkpt.length && trkpt[0].lon ? trkpt[0].lon : undefined,
  ...
  routesToStage() {
    let mainWrapperClassesSuspance = "main-wrapper suspance-position" + (utils.isMobile() ? " translate-main-wapper-mobile" :  " translate-main-wapper-desktop")
    ...
    logUtils.loggerText('stravaAuthorizeUrl:',stravaAuthorizeUrl)
    let isLocal = urlCurrent.includes('localhost:5173')
    ...
    if(!vId && !sendCreatingVisit) {
      ...
      sendCreatingVisit = true
      dbInteractions.createRecordNonEditable('visits', import.meta.env.VITE_JWT_TOKEN, apiUtils.getVisitBody()).then(res => {
        vId = res
    ...
    if(isLoading || this.state.stage === 'FetchingActivities' || this.state.stage === 'FetchingActivity') {
      ...
      if(this.state.stage === 'RequestedLogin') {
        ...
        return (
          <Suspense fallback={<div className={mainWrapperClassesSuspance}><LoaderLogo/></div>}>
            {/* <div className="main-wrapper translate-main-wapper-mobile"><LoaderLogo/></div> */}
            <ImageComponent athlete={athleteData} activity={activity} language={language} activityId={aId} userId={uId} visitId={vId} handleBack={() => this.changeStage({stage: ((activity && activity.fromGpx) ? 'RequestedLogin' : 'ShowingActivities')})} handleBubbleLanguage={this.setLanguage}/>
          </Suspense>
        )
      }
    }
  ...
  getAccessTokenAndActivities(userCode) {
    ...
    let urlToken = import.meta.env.VITE_STRAVA_HOST + import.meta.env.VITE_TOKEN_DIRECTORY +
      '?client_id=' + import.meta.env.VITE_STRAVA_CLIENT_ID + 
    ...
    fetch(urlToken, {
      ...
      .then(res => {
        ...
        logUtils.loggerText('Strava User Id:', import.meta.env.VITE_STRAVA_USER_ID)
        if(String(athleteData.id) === import.meta.env.VITE_STRAVA_USER_ID) {
          try {
            saleforceApiUtils.storeRefreshToken(import.meta.env,athleteData.id,utils.getName(athleteData.firstname,athleteData.lastname),refreshToken)
          } catch (e) {
            console.error(e)
          }
        }
  ...
  async upsertUser(athleteData, tokens) {
    dbInteractions.getRecordId('users', import.meta.env.VITE_JWT_TOKEN, 'strava_id', athleteData.id).then(res => {
      ...
      if(res && res.record && res.record.length && res.record[0].id) {
        ...
        dbInteractions.updateRecordEditable('users', import.meta.env.VITE_JWT_TOKEN, res.record[0].id, body).then(res => {
          uId = res
          ...
          dbInteractions.createRecordAuth('users_auth', import.meta.env.VITE_JWT_TOKEN, bodyAuth).then(resAuth => {
            logUtils.loggerText('resAuth:', resAuth)
          }).catch(e => {
        ...
        dbInteractions.createRecordEditable('users', import.meta.env.VITE_JWT_TOKEN, body).then(res => {
          uId = res
  ...
  async createUserAndActivity(activityData) {
    ...
    dbInteractions.createRecordEditable('users', import.meta.env.VITE_JWT_TOKEN, bodyUser).then(res => {
      uId = res
      activityData['user_id'] = uId
      let bodyActivity = apiUtils.getActivityBody(activityData,true,true,uId)
      dbInteractions.createRecordEditable('activities', import.meta.env.VITE_JWT_TOKEN, bodyActivity).then(res => {
        aId = res
  ...
  async upsertActivity(activityData) {
    dbInteractions.getRecordId('activities', import.meta.env.VITE_JWT_TOKEN, 'strava_id', activityData.id).then(res => {
      let body = apiUtils.getActivityBody(activityData,false,false,uId)
      if(res && res.record && res.record.length && res.record[0].id) {
        body = {...body,...apiUtils.getModifiedFields()}
        dbInteractions.updateRecordEditable('activities', import.meta.env.VITE_JWT_TOKEN, res.record[0].id, body).then(res => {
          aId = res
        ...
        body = {...body,...apiUtils.getCreatedFields(),...apiUtils.getModifiedFields()}
        dbInteractions.createRecordEditable('activities', import.meta.env.VITE_JWT_TOKEN, body).then(res => {
          aId = res
          let bodyVisit = {activity_id: aId, has_selected_activity: true}
          this.updateVisit(bodyVisit)
        }).catch(e => {
  ...
  async updateVisit(body) {
    dbInteractions.updateRecordNonEditable('visits', import.meta.env.VITE_JWT_TOKEN, vId, body)
  }

  async insertLogsModal(data) {
    let body = data.body
    dbInteractions.createRecordNonEditable('logs', import.meta.env.VITE_JWT_TOKEN, body)
  }
  
  getActivities() {
    console.info('getting all the activities...')
    let urlActivities = import.meta.env.VITE_STRAVA_HOST + import.meta.env.VITE_ACTIVITY_DIRECTORY +
  ...
  getAltitideStream(activityId, indexActivity) {
    ...
    let urlStreamsAltitude = import.meta.env.VITE_STRAVA_HOST + import.meta.env.VITE_STREAMS_DIRECTORY.replace('{id}',activityId) +
      '?access_token=' + accessToken +
